...
export const trackedShows = shows => Object.values(shows).filter(show => !show.dropped);

export const droppedShows = shows => Object.values(shows).filter(show => show.dropped).sort(byName);

export const sectionsOf = shows => SECTIONS.map(section => ({
  ...section,
...
export const upcomingMonths = shows => {
  const entries = trackedShows(shows)
    .flatMap(show => (show.upcoming ?? []).map(episode => ({ episode, show })))
  ...
  for (const { episode, show } of entries){
    ...
    months.get(month).push({ ...episode, label: show.name, key: `${show.id}:${episode.airDate}` });
  }
