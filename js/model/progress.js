...
export const isEnded = show => show.tvStatus === "Ended" || show.tvStatus === "Canceled";
...
export const episodesBehind = show => {
  ...
  for (const season of show.seasons){
    if (season.number < show.currentSeason || season.number > show.last.season) continue;
    const aired = season.number === show.last.season ? show.last.episode : season.episodeCount;
    const watched = season.number === show.currentSeason ? show.currentEpisode : 0;
...
export const sectionOf = show => {
  ...
  if (show.currentEpisode === 0 && show.currentSeason <= firstSeason) return "notStarted";
...
export const nextWatched = show => {
  ...
  if (!season || episode <= season.episodeCount) return { season: show.currentSeason, episode };
...
export const previousWatched = show => {
  if (show.currentEpisode > 1) return { season: show.currentSeason, episode: show.currentEpisode - 1 };

  const earlier = [...show.seasons].reverse().find(s => s.number < show.currentSeason);
  return earlier
...
export const allWatched = show => show.last
...
const absolute = (show, at) => show.seasons
...
export const namedSeasons = show => [...new Set([show.currentSeason, nextWatched(show).season])];

// Whether the record carries the episodes of every season the card will name.
export const knowsNamedSeasons = show =>
  namedSeasons(show).every(number => show.episodes?.some(episode => episode.season === number));

// The title of one episode, from the seasons the record carries.
export const titleAt = (show, at) => [...(show.episodes ?? []), show.last, ...(show.upcoming ?? [])]
  .find(episode => episode && episode.season === at.season && episode.episode === at.episode)?.title ?? "";

export const watchedAt = show => ({ season: show.currentSeason, episode: show.currentEpisode });
...
export const seasonWatched = show => {
  ...
  return {
    season: at.season,
    episode: show.last?.season === at.season ? show.last.episode : season.episodeCount
  };
};

export const seasonLeft = show =>
  Math.max(0, absolute(show, seasonWatched(show)) - absolute(show, watchedAt(show)));
