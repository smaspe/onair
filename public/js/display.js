...
export const display = {
  ...
  sizeOf: (row) => {
    ...
    const seasons =
      row.seasonTotal === 1 ? "1 season" : `${row.seasonTotal} seasons`;
    return [row.kind, seasons, `${row.episodeTotal} episodes`]
      .filter(Boolean)
      .join(" · ");
  },
  ...
  airing: (show) => {
    ...
    return (
      next && { label: `${codeAt(next)} ${airsIn(next)}`, soon: airsSoon(next) }
    );
  },
};
